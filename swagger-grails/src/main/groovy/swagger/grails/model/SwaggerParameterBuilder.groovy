...
@Slf4j
class SwaggerParameterBuilder extends SwaggerBuilderHelper {
    /**
     ...
     */
    static List<SwaggerParameter> buildSwaggerParameters(String nickname, DefaultGrailsControllerClass controllerClass, String actionName, UrlMapping urlMapping) {
        List<String> pathParams = buildPathParams(urlMapping)

        if(!pathParams) {
            def matcher = (nickname =~ /\{([^}]+)\}/)
            pathParams = matcher.collect { it[1] }.<String>toList()
        }
